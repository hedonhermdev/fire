...
class AddPageForm extends React.Component {
    ...
    submitAddPage = (e) => {
        ...
        if (window.location.pathname !== '/Manage_Content') {
            parentGroupId = window.location.pathname.split('/')[window.location.pathname.split('/').length - 1];
        }

        const formData = new FormData(e.target);
        formData.append('parentGroupId', parentGroupId);

        axios.post(api.ADD_PAGE, formData, {
            headers: {
...
export default connect(mapStateToProp, mapDispatchToProps)(AddPageForm);